...
.PHONY: build build-c-lib build-daemon run-engine run-mcp healthcheck log-debug log-info install install-fff uninstall test test-rust test-rescan test-rescan-known-defects rescan-probe test-daemon test-c-smoke test-c-api test-lua test-lua-snap test-version test-bun test-node prepare-bun prepare-bun-packaged prepare-node set-npm-version header test-stress test-stress-seeded test-stress-random test-stress-regressions test-stress-repos test-node-stress sync-js-api sync-js-api-check bump-homebrew-formula bump-install-mcp-sh test-bun-compile
